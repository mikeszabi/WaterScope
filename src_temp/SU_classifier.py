...
class params:
    ...
    def __init__(self,cfg_file='cfg_SU_classifier.xml'):
        ...
        if os.path.exists(cfg_file):
            ...
            self.files['thresholds'] = int(tree.find('files/thresholds').text)

#            self.threshold_taxon=self.get_thresholds(self.files['typedict'])
            ...
            for mfo in measure_folders:
                ...
                for index, df_image in df_measure.iterrows():
                    ...
                    if self.pred_strength_out:
                        prob_str='{0:0{1}d}'.format(int(round(df_image['prob_taxon'])),5)+'_'
                    else:
                        prob_str=''
